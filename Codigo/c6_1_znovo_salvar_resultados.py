...
def gerar_planilha_nova(dados, caminho_resultado):
    ...
    # Valor constante para a coluna 'cenario'
    ...
    for label_dataset, resto_dataset in dados.items():
        ...
        # O dicionário agora é estruturado como: resto_dataset[tecnica][modelo]
        # Vamos iterar pelas técnicas primeiro
        for tecnica_nome, modelos_da_tecnica in resto_dataset.items():
            
            # Pula a chave 'smote' que está no mesmo nível das técnicas
            if tecnica_nome == 'smote' or not isinstance(modelos_da_tecnica, dict):
                continue

            # Agora iteramos pelos modelos dentro de cada técnica
            for modelo, metricas_tecnica in modelos_da_tecnica.items():
                ...
                # Para técnicas de pós-processamento, buscamos os parâmetros do modelo original (sem_tecnica)
                # No novo padrão, acessamos: resto_dataset['sem_tecnica'][modelo]
                parametros_base_dict = resto_dataset.get('sem_tecnica', {}).get(modelo, {}).get('melhores_parametros')
                ...
                # Definido como hífen conforme solicitado
                tempo_cpu = '-'
                ...
                # =========================================================
                if feature_sensivel_base in MAP_SENSIBLE:
                    grupos_status = {
    ...
    # --- Criar e Salvar DataFrames (Lógica de salvamento original mantida) ---
    ...
    ordem_resultados = ['dataset', 'modelo', 'tecnica', 'cenario', 'smote', 'tempo_cpu', 'acuracia_teste', 'recall_teste', 'precisao_teste', 'f1_teste', 'auc_teste', 'ks_teste', 'classification_report_teste', 'confusion_matrix_teste']
    ordem_extended = ['dataset', 'cenario', 'smote', 'modelo', 'tecnica', 'feature', 'demographic_parity_difference', 'demographic_parity_ratio', 'equalized_odds_difference', 'equalized_odds_ratio', 'equal_opportunity_difference', 'equal_opportunity_ratio', 'false_positive_rate_difference', 'false_positive_rate_ratio', 'false_negative_rate_difference', 'false_negative_rate_ratio', 'predictive_parity_difference', 'predictive_parity_ratio']
    ordem_fairness_results = ['dataset', 'cenario', 'smote', 'modelo', 'tecnica', 'feature', 'group', 'count', 'accuracy', 'recall', 'precision', 'f1', 'confusion_matrix', 'true_positive_rate', 'true_negative_rate', 'false_positive_rate', 'false_negative_rate', 'selection_rate']
    ordem_parametros = ['dataset', 'modelo', 'tecnica', 'cenario', 'smote', 'melhores_parametros']

    df_resultados_completos = df_resultados_completos[[c for c in ordem_resultados if c in df_resultados_completos.columns]]
    df_extended_fairness = df_extended_fairness[[c for c in ordem_extended if c in df_extended_fairness.columns]]
    df_fairness_results = df_fairness_results[[c for c in ordem_fairness_results if c in df_fairness_results.columns]]
    df_parametros_completos = df_parametros_completos[[c for c in ordem_parametros if c in df_parametros_completos.columns]]
